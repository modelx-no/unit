:lang: no
// include::architecture-repository:common:partial$commonincludes.adoc[]

// :lang: no ------------>
ifeval::["{lang}" == "no"]

//* xref:unit-ra:ROOT:index.adoc[Om UNIT referansearkitekturer]

* xref:index.adoc[Om UNIT referansearkitekturer]

* xref:main@unit-ra:Bøker:page$Referansearkitektur for deling av data i høyere utdanning og forsking.adoc[]

** xref:main@unit-ra:unit-ra-datadeling-introduksjon:page$Introduksjon.adoc[Introduksjon]
** xref:main@unit-ra:unit-ra-datadeling-tilnærming:page$Tilnærming til utvikling av referansearkitekturen.adoc[]
** xref:main@unit-ra:unit-ra-datadeling-målarkitekturen:page$Datadelingsplattformen og felles komponentene i den.adoc[]
** xref:main@unit-ra:unit-ra-datadeling-datautveksling:page$Datautveksling.adoc[]
** xref:main@unit-ra:unit-ra-datadeling-rutiner:page$Rutiner.adoc[]
** xref:main@unit-ra:unit-ra-datadeling-roller:page$Roller og ansvar for informasjonsforvaltning.adoc[]
include::main@unit-ra:unit-ra-datadeling-definisjoner:page$Vedlegg - Definisjoner.adoc[Vedlegg A: Definisjoner]
include::main@unit-ra:unit-ra-datadeling-issues:page$Vedlegg - Behov for avklaringer og utfordringer i sektorens kontekst.adoc[Vedlegg B: Behov for avklaringer og utfordringer i sektorens kontekst]
include::main@unit-ra:unit-ra-datadeling-muliggjørende-økosystem:page$Vedlegg - Muliggjørende økosystem.adoc[Vedlegg C: Muliggjørende økosystem]
include::main@unit-ra:unit-ra-datadeling-behov-og-bruksområder:page$Vedlegg - Behov og bruksområder.adoc[Vedlegg D: Behov og bruksområder]
include::main@unit-ra:unit-ra-datadeling-føringer:page$Vedlegg - Føringer, pågående og videre arbeid.adoc[Vedlegg E: Føringer, pågående og videre arbeid]


endif::[]
// :lang: no <-----------
 


// :lang: en ------------>
ifeval::["{lang}" == "en"]

* xref:index.adoc[About unit-ra]

** xref:index.adoc[Welcome]

endif::[]
// :lang: en <-----------
